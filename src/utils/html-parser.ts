...
import { ComponentStyles, CSSRule, DesignToken } from '../types/storybook.js';

export function extractComponentStyles(html: string, styleSheets: string[] = []): ComponentStyles {
...
function extractCustomPropertiesFromStyle(style: string, customProperties: Record<string, string>): void {
...
function extractCustomPropertiesFromCSS(css: string, customProperties: Record<string, string>): void {
  const propertyRegex = /--([\w-]+):\s*([^;]+)/g;
  let match;
...
function categorizeToken(name: string, value: string): DesignToken['type'] {
  ...
  if (lowerName.includes('space') || lowerName.includes('margin') || lowerName.includes('padding') ||
      lowerName.includes('gap') || lowerName.includes('size') ||
      /^\d+(px|rem|em|%)$/.test(value)) {
    return 'spacing';
  }
  
  if (lowerName.includes('font') || lowerName.includes('text') ||
      lowerName.includes('weight') || lowerName.includes('family')) {
    return 'typography';
  }
